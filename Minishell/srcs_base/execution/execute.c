...
static	void	child_process(t_shell *shell, int *prev_read_fd, int *fd, int i)
{
	...
	{
		ret = exec_builtin(shell->cmds[i].ac, shell->cmds[i].final_args, shell);
		cleanup_exit(shell, ret);
	}
	path = get_line_to_exec(shell->cmds[i].final_args[0], shell->env);
	if (!path)
	{
		ft_fprintf(2, E_MS"%s, "E_CMD, shell->cmds[i].final_args[0]);
...
pid_t	execute(t_cmd *cmd, t_shell *shell, int *prev_read_fd, int i)
{
	...
	pid = fork();
	if (pid < 0)
		return (error_return(E_MS, "Error creating child", -1));
	else if (pid == 0)
